...
export async function addFavoriteSong(req, res) {
  ...
  try {
    ...
    if (channelId)
    {
      ...
      if (artist.name !== channelName || artist.avatar !== enriched.artistAvatar) {
        await artist.update({ name: channelName, avatar: enriched.artistAvatar });
      }
      artistId = artist.id;
    }
...
export async function removeFavoriteSong(req, res) {
  ...
  try
  {
    ...
    if (song) await FavoriteSong.destroy({ where: { userId, songId: song.id } });
    res.json({ ok: true });
  }
  catch (err) {
